class_name MagicDatabase
extends RefCounted

## Arvore de MAGIA — metade magica da antiga arvore unica de TechDatabase.gd,
## separada estruturalmente dela (pedido do usuario: "separar conceitualmente
## e tecnicamente: 1. ARVORE DE TECNOLOGIA 2. ARVORE DE MAGIA... duas
## progressoes completamente independentes"). Contem as 7 escolas magicas
## (Arcanismo, Alquimia, Transmutacao, Naturalismo, Geomancia, Elementalismo,
## Necromancia) — a escola "Doutrina" (mundana) ficou em TechDatabase.gd.
## Nenhum id/custo/pre-requisito/bonus/unlock mudou nesse corte, so a
## localizacao do dado. Ver TechDatabase.gd pra explicacao completa da
## arvore mundana e PlayerData.gd/GameManager.gd pra como o progresso das
## duas arvores e mantido separado (researched_techs vs researched_magic,
## mesmo slot unico de pesquisa ativa pros dois — ver comentario de
## GameManager._process_research).
##
...
## (ver GameManager._process_research) — mesmo mecanismo da arvore mundana.
...
static func get_tech(id: String) -> TechData:
	if _current.is_empty():
		_current = MagicContent.build_techs()
	if _current.has(id):
		return _current[id]
...
static func school_level(school: String, researched: Dictionary) -> int:
	for tier in range(9, 0, -1):
		if researched.has("%s_%d" % [school, tier]):
			return tier
	return 0

## Conserva capacidades antigas e concede a linha equivalente; não infere por nomes.
static func migrate_legacy_research(player: PlayerData) -> void:
	var remap := {
		"canalizacao_base": ["arcanismo", 1], "invocacao_espiritos": ["arcanismo", 3],
		"alquimia_botanica": ["druidismo", 1], "pacto_florestal": ["druidismo", 6],
		"transmutacao_rocha": ["arcanismo", 1], "forja_runica": ["arcanismo", 6],
		"geomancia": ["druidismo", 4], "lordes_dos_ventos": ["elementalismo", 3],
		"necromancia_pratica": ["necromancia", 5], "constructos_de_guerra": ["arcanismo", 5],
		"cataclismo_elemental": ["elementalismo", 7], "transcendencia_florestal": ["druidismo", 7],
	}
	for old_id in remap:
		if player.researched_magic.has(old_id):
			for tier in range(1, int(remap[old_id][1]) + 1):
				player.researched_magic["%s_%d" % [remap[old_id][0], tier]] = true
	# Pesquisas em curso continuam com o id e os pontos originais até concluírem.

## Mesma logica de TechDatabase.available_techs — tecnologias magicas ainda
## nao pesquisadas cujos pre-requisitos ja foram TODOS cumpridos.
static func available_techs(researched: Dictionary) -> Array:
	var result := []
	for tech in all_techs():
		if researched.has(tech.id):
			continue
		if tech.id == "transcendencia_arcana":
			var mastered := 0
			for school in MagicContent.SCHOOLS:
				if school_level(school, researched) == 9:
					mastered += 1
			if mastered < 2:
				continue
		var ok := true
...
## Consulta de compatibilidade para saves anteriores; a árvore nova não a exibe.
static func available_legacy_techs(researched: Dictionary) -> Array:
	return _all().values().filter(func(t): return not researched.has(t.id) and t.prerequisites.all(func(id): return researched.has(id)))

## Mesma semantica fail-open de TechDatabase.is_unit_unlocked: kind sem
## nenhuma tech magica associada fica liberado por padrao (quem decide se
## kind pertence a outra arvore e PlayerData.has_unlocked, que tenta
## TechDatabase primeiro).
static func is_unit_unlocked(kind: String, researched: Dictionary) -> bool:
	for tech in all_techs() + _all().values():
...
## Nomes de todos os feiticos/rituais concedidos pelas tecnologias magicas
## ja pesquisadas — unica fonte de feiticos no jogo (nenhuma tech de
## Doutrina concede feitico, ver TechDatabase.gd).
