...
##     cidade, importancia (populacao), covil proximo e tropas proprias ja
##     guarnecendo. Um goblin solitario a 5 tiles rende ~0.17 (so' um leve
##     empurrao na producao); 3 esqueletos a 4 tiles rendem ~0.9 (prioridade
##     defensiva). Usado por RivalAI.decide_production (producao emergencial)
##     e defend_turn abaixo.
...
##  3. militia_strike() -- defesa PROPRIA da cidade (vale pra jogador e IA,
##     unico sistema defensivo automatico do jogo): golpe fraco por turno num
##     monstro ADJACENTE. Impede o cerco eterno absurdo, mas nunca mata um
##     troll sozinha nem da' recompensa (sem farm).
...
## Milicia da cidade (defesa automatica). Deliberadamente fraca: um
## Esqueleto (6 HP) cai em ~3 turnos ao lado de uma vila, um Troll (20 HP)
## nem sonha em cair sozinho. Muralhas somam um pouco.
const MILITIA_BASE_DAMAGE := 1.5
const MILITIA_DAMAGE_PER_POPULATION := 0.25
const MILITIA_MAX_BASE_DAMAGE := 3.0
const MILITIA_WALLS_BONUS := 1.0
...
static func assess(city: City, hex_grid: HexGrid, visible: Variant = null) -> Dictionary:
	...
	# Importancia estrategica (populacao) e fragilidade (vida+escudo) da
	...
	var stakes := (1.0 + 0.5 * (1.0 - health_fraction)) * (1.0 + 0.04 * minf(float(city.population), 10.0))
...
## Dano do golpe automatico da cidade (ver constantes MILITIA_*).
static func militia_damage(city: City) -> float:
	var damage := minf(MILITIA_BASE_DAMAGE + MILITIA_DAMAGE_PER_POPULATION * float(city.population), MILITIA_MAX_BASE_DAMAGE)
	if city.buildings.has("walls"):
		damage += MILITIA_WALLS_BONUS
	return damage

## Defesa propria da cidade (jogador OU IA): um golpe por turno no monstro
## ADJACENTE mais ferido. Sem recompensa (ouro/veterania) de proposito -- e'
## uma milicia, nao uma fonte de farm. Devolve o monstro atingido (ou null).
static func militia_strike(city: City, hex_grid: HexGrid) -> Unit:
	var target: Unit = null
	for neighbor_coord in hex_grid.get_neighbors(city.coord):
		var unit: Unit = hex_grid.get_unit_at(neighbor_coord)
		if unit == null or unit.owner_player != null or unit.world_event_managed:
			continue
		if target == null or unit.hp < target.hp:
			target = unit
	if target == null:
		return null
	var damage := militia_damage(city)
	var coord := target.coord
	var monster_name := target.unit_data.unit_name
	target.hp -= damage
	hex_grid.spawn_damage_popup(coord, damage)
	var is_human := city.owner_player == GameManager.human_player
	if target.hp <= 0.0:
		hex_grid.remove_unit(target)
		if is_human:
			EventBus.notify.emit("A milicia de %s abateu um %s!" % [city.city_name, monster_name], "combat")
	elif is_human:
		EventBus.notify.emit("A milicia de %s feriu um %s cercando a cidade." % [city.city_name, monster_name], "combat")
	return target
