...
Regras arquiteturais explícitas, fixadas antes de escrever a primeira linha do `WorldEventManager` — o objetivo é que o World Event nunca vire um grande bloco especial dentro do `GameManager`, e que persistência não seja retrofitada depois de o sistema já existir.
