...
- Teste obrigatório desde o primeiro commit do Dragon (sequência completa, não só o resultado final): criar o evento → colocar em `Preparation`/`Active`/`Resolution` → salvar → carregar → verificar mesma `phase` → verificar mesmo estado específico → verificar que **nenhum avanço ocorreu durante o load** → só então avançar um turno de verdade → verificar que a evolução só acontece a partir daí. O último ponto idealmente é um teste que detecta qualquer chamada indevida a `advance_turn()` durante `from_save_dict()` diretamente (ex.: um contador de chamadas no stub/evento usado no teste), não apenas verificar que a fase permaneceu igual — protege a causa (a chamada nunca aconteceu), não só o efeito (a fase por acaso não mudou). Esta garantia só é observável a partir do momento em que existe um evento concreto reconstruível — não é uma lacuna do Step 3 (persistência genérica), que já cobre tudo que é verificável sem um tipo concreto (`_next_event_id` sobrevivendo e nunca colidindo, principalmente).
