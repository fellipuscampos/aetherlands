...
## Cobre DragonEvent (Fase Macro, Step 5A -- SKELETON): prova o mecanismo
## generico (FSM/persistencia/determinismo, ver docs/WORLD_EVENT_CONTRACT.
## md) com um tipo CONCRETO real, antes de qualquer regra de jogo do
## Dragao (spawn/alvo/combate/recompensa -- Step 5B, ainda nao existe).
...
func test_new_dragon_event_starts_dormant_with_no_lair_coord():
	var event := DragonEvent.new()
	assert_eq(event.phase, WorldEvent.PHASE_DORMANT)
	assert_eq(event.lair_coord, DragonEvent.NO_LAIR_COORD)
...
func test_to_save_dict_and_from_save_dict_round_trip_lair_coord():
	var event := DragonEvent.new()
	event.phase = WorldEvent.PHASE_ACTIVE
	event.lair_coord = Vector2i(7, -3)
	...
	assert_eq(loaded.lair_coord, Vector2i(7, -3))

func test_from_save_dict_defaults_lair_coord_when_missing():
	var event := DragonEvent.new()
...
func test_world_event_manager_reconstructs_a_dragon_event_from_save_dict():
	...
	assert_eq((reconstructed as DragonEvent).lair_coord, Vector2i(2, 9))
...
## nada, e que o estado do evento (lair_coord) nao interfere no RNG.
func test_dragon_event_rng_is_deterministic_and_independent_of_its_own_state():
	var event_a := DragonEvent.new()
	event_a.event_id = 4
	event_a.lair_coord = Vector2i(1, 1)
	var event_b := DragonEvent.new()
	event_b.event_id = 4
	event_b.lair_coord = Vector2i(50, 50) # estado especifico diferente, mesmo event_id

	assert_eq(event_a.event_rng(777, 10).randi(), event_b.event_rng(777, 10).randi(), "o RNG do evento depende so de map_seed+event_id+turno, nunca do proprio estado especifico (lair_coord)")
